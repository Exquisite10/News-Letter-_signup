...
//     }
//     else{
//         child.innerHTML=`<img src="assets/images/cropped.png">`;
...
width=window.outerWidth;
    ...
    let main_page_remove=document.querySelector(".container");
    let data_container=document.querySelector(".data-container");
    const valid=data_container.children[3]
    // console.log(valid);
    let input=data_container.children[4];
    
    let button=data_container.children[5];

    submit=()=>{
        mail=input.value;

        const regex=/\w+@[a-zA-Z0-9]+\.(com|net|org|yahoo|co|)/;
        let f=document.createElement("span");
        f.innerContent="Valid email required";
        f.classList.add("validate");
        console.log(f.innerContent);

        if(mail==""){
            // console.log(mail);
            // console.log(valid.children[0]);
            if(valid.childElementCount === 0){

                valid.appendChild(f);
                console.log("iniseeeeeeeeeee if")
            }
        }
            ...
            if(valid.childElementCount === 0){
                let f=document.createElement("span");
                f.innerText="Valid email required";
                f.classList.add("validate");
                valid.append(f);
            }
            ...
            main_page_remove.remove();
            let space=document.querySelector("body");
            let script=document.querySelector("script");
            const new_container=document.createElement("div");
            space.insertBefore(new_container,script);
            let baby1=document.createElement("div");
            let baby2=document.createElement("div");
            let baby3=document.createElement("div");
            let button1=document.createElement("button");
            new_container.append(baby1);
            baby1.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21"><g fill="none"><circle cx="10.5" cy="10.5" r="10.5" fill="#FF6155"/><path stroke="#FFF" stroke-width="2" d="M6 11.381 8.735 14 15 8"/></g></svg>`;
            new_container.append(baby2);
            baby2.innerText=`Thanks for subscribing!`
            
            let summary=`A confirmation email has been sent to ${mail}. Please open it and click the button inside to confirm your subscription.` ;

            let strong=document.createElement("strong");
            strong.innerText=mail;

            let location=summary.indexOf(mail);
            let mailLength=mail.length;

            let first=summary.substring(0,location);
            let second =summary.substring(location+mailLength,summary.length);
            baby3.innerHTML=first + strong.outerHTML + second;
            
            new_container.append(baby3);


            // let completeString=first




            new_container.append(button1);
            button1.innerText="Dismiss message"



            

            new_container.classList.add("new_container");


            button1.classList.add("stke");
            body.classList.add("main");
            color=()=>{
                console.log("hello");
                button1.classList.remove("stke");
                button1.style.backgroundColor="hsl(234, 29%, 20%)";
